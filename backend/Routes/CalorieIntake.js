...
//         },
//     }, async function (error, response, body) {
//         if (error) return res.status(500).json(createResponse(false, 'API request failed'));
//         if (response.statusCode !== 200) return res.status(response.statusCode).json(createResponse(false, 'API error', body));

//         body = JSON.parse(body);
//         if (!Array.isArray(body) || body.length === 0) {
//             return res.status(404).json(createResponse(false, 'No nutrition data found'));
//         }

//         const nutrition = body[0];
//         const caloriesPerGram = nutrition.calories / nutrition.serving_size_g;
//         const calorieIntake = parseInt(caloriesPerGram * qtyingrams);
...
//     const entries = filterEntriesByDate(user.calorieIntake, targetDate);
//     res.json(createResponse(true, 'Calorie intake for the date', entries));
// });
...
//     const date = new Date();
//     const cutoff = new Date(date.setDate(date.getDate() - parseInt(limit))).getTime();
//     const filtered = user.calorieIntake.filter(item => new Date(item.date).getTime() >= cutoff);
//     res.json(createResponse(true, `Calorie intake for the last ${limit} days`, filtered));
// });
...
//     user.calorieIntake = user.calorieIntake.filter(entry => {
//         return !(entry.item === item && new Date(entry.date).getTime() === new Date(date).getTime());
//     });
...
//     let maxCalorieIntake = user.goal === 'weightLoss'
//         ? BMR - 500
